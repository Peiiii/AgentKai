...
export class CLIConfigService {
  ...
  async createOrEditConfig(): Promise<void> {
    try {
      ...
      // 检查配置文件是否存在
      const configFiles = this.coreConfigService.findConfigFiles();
      ...
      if (keyAnswer.apiKey) {
        console.log('\n正在保存API密钥...');
        // 如果用户输入了API密钥，直接保存
        const result = this.coreConfigService.saveConfigEntry({ AI_API_KEY: keyAnswer.apiKey });
        if (result) {
  ...
  async handleConfigCommand(options: any, commandArgs: string[]): Promise<void> {
    ...
    if (options.edit) {
      ...
      if (configFiles.length === 0) {
        configPath = this.coreConfigService.createDefaultUserConfig();
        console.log(`已创建配置文件: ${configPath}`);
    ...
    if (options.set) {
      ...
      const result = this.coreConfigService.saveConfigEntry({ [key]: value });
      if (result) {
